...
function promptBody(prompt, files, contextEnvironment, artifactDirectory) {
  ...
  instructions.push(`When asked to restart Pipa, run \`pipa restart\`; existing messaging authorization is sufficient. It requests a detached restart and may end this turn. Use \`pipa restart --status\` to inspect the durable outcome; do not claim completion from the request alone. The Pipa profile home is ${JSON.stringify(path.resolve(process.env.PIPA_HOME || os.homedir()))}; ensure PIPA_HOME has this value in the command environment, including on attached servers. Do not use sequential stop/start or OpenCode /instance/dispose: that disposes an instance, not restarts the server.`);
