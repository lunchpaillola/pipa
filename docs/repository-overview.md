...
3. During Socket Mode startup, `src/app.mjs` checks that `PIPA_OPENCODE_ATTACH_URL` or one owned OpenCode server on loopback port `0` can serve the configured workspace, then creates the Slack connection and restores previously subscribed Slack threads from `~/.pipa/sessions.json`. During Managed startup, `src/opencode.mjs` starts one `opencode serve` child with the profile's working directory, hostname, and port.
