...
5. `src/opencode.mjs` downloads attachments through Chat SDK into a temporary directory, sends the prompt, Slack context, summary-first delivery guidance, and native file parts through OpenCode's session API, and polls authoritative status and message endpoints until completion. Files declared by a filesystem-local OpenCode turn are opened from a private per-turn directory and checked for path, type, identity, and size safety. Non-loopback attached servers do not receive the local artifact contract. Temporary files are removed on every terminal path.
