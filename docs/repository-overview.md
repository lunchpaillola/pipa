...
- `src/mcp.mjs`: Owns the in-process MCP bridge around the Chat SDK reader tools. `createMcpBridge` wraps the `createChatTools({ preset: "reader" })` tools object, serves `initialize`, `tools/list`, and `tools/call` over JSON-RPC, validates arguments against each source tool schema before `execute`, and returns bounded, credential-safe results. Only the seven read-only tool names are ever exposed.
...
- `test/mcp.test.mjs`: Tests the MCP bridge handshake, reader-only tool listing with preserved schemas, validated calls, credential-safe errors, bounded results, and JSON-RPC handling.
