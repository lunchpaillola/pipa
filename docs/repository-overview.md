...
7. Due routines bypass conversation continuity and call the same executor with a fresh session, exact saved prompt, configured working directory, and saved Slack channel or thread. The scheduler permits different routines to overlap but never overlaps one routine with itself. Every dispatch, interaction, response, and final delivery reloads the relevant authorization. Edits affect future work; deactivation, deletion, destination revocation, and shutdown abort active work and suppress stale delivery.
